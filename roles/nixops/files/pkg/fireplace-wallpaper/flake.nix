    ...
    let
      ...
      mainQml = pkgs.writeText "main.qml" ''
        ...
        Item {
            ...
            property real _netRx: 0
            property real _netTx: 0
            property string cpuStat: "--%"
            property string gpuStat: "--%"
            property string memStat: "--/--G"
            property string dskStat: "r-- w-- MB/s"
            property string netStat: "d-- u-- MB/s"

            Component.onCompleted: {
                var s = Qt.resolvedUrl("start-audio.sh").toString().replace(/^file:\/\//, "")
                audioSource.connectSource("sh " + s)
            }

            Component.onDestruction: {
                var s = Qt.resolvedUrl("stop-audio.sh").toString().replace(/^file:\/\//, "")
                audioSource.connectSource("sh " + s)
            }
                ...
                onNewData: function(source, data) {
                    ...
                    for (var i = 0; i < lines.length; i++) {
                        ...
                        if (line.indexOf("CPU:") === 0) {
                            ...
                            var np = line.slice(4).split(" ")
                            var rx = +np[0], tx = +np[1]
                            if (root._netRx > 0)
                                root.netStat = "d" + ((rx-root._netRx)/2097152).toFixed(2) + " u" + ((tx-root._netTx)/2097152).toFixed(2) + " MB/s"
                            root._netRx = rx
                            root._netTx = tx
                        }
                    }
            ...
            Item {
                ...
                property real bobY: 80

                SequentialAnimation on bobY {
                    loops: Animation.Infinite
                    running: true
                    NumberAnimation { to: 77; duration: 1750; easing.type: Easing.InOutSine }
                    NumberAnimation { to: 83; duration: 1750; easing.type: Easing.InOutSine }
                }
                ...
                SequentialAnimation on y {
                    ...
                    running: true
                    NumberAnimation {
                        to: root.height * 0.667 - 80
                        duration: 7500
                        easing.type: Easing.InOutSine
                    }
                    NumberAnimation {
                        to: root.height * 0.667 - 72
                        duration: 7500
                        easing.type: Easing.InOutSine
                    }
                }
                ...
                // Boat hull: wide flat deck tapering to a curved keel (rendered last = on top)
                Canvas {
                    id: hull
                    width: 140
                    height: 36
                ...
                // Fishing line (curved bezier from rod tip into water)
                Canvas {
                    x: 151
                    y: 18
                    width: 20
                    height: boatScene.bobY - 18 + 3
                    ...
                    onPaint: {
                        ...
                        ctx.beginPath()
                        ctx.moveTo(3, 0)
                        ctx.quadraticCurveTo(-4, height / 2, 4, height)
                        ctx.strokeStyle = "rgba(192, 200, 216, 0.8)"
                        ctx.lineWidth = 1
                        ctx.stroke()
                    }
                }

                // Bobber — bobs with wave animation
                Canvas {
                    width: 6
                    height: 6
                    x: 152
            ...
            // Clock and diagnostics — rendered last, always on top
            Column {
                id: clock
                spacing: 6
      ...
      startAudioScript = pkgs.writeText "start-audio.sh" ''
        #!/bin/sh
        kill $(cat /tmp/screensaver-mpv.pid 2>/dev/null) 2>/dev/null
        mpv --no-video --really-quiet --volume=50 https://rainwave.cc/tune_in/2.ogg.m3u &
        echo $! > /tmp/screensaver-mpv.pid
      '';

      stopAudioScript = pkgs.writeText "stop-audio.sh" ''
        #!/bin/sh
        kill $(cat /tmp/screensaver-mpv.pid 2>/dev/null) 2>/dev/null
        rm -f /tmp/screensaver-mpv.pid
      '';
    in
    {
      ...
      packages.${system}.default = pkgs.stdenv.mkDerivation {
        ...
        installPhase = ''
          ...
          install -m755 ${startAudioScript} "$dest/contents/ui/start-audio.sh"
          install -m755 ${stopAudioScript}  "$dest/contents/ui/stop-audio.sh"
