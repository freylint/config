...
in
  {
    wincompat-has-bottles      = scenario "wincompat-has-bottles"      (builtins.elem pkgs.bottles                  cfg.environment.systemPackages);
    wincompat-has-wine-staging = scenario "wincompat-has-wine-staging" (builtins.elem pkgs.wineWow64Packages.staging cfg.environment.systemPackages);
    wincompat-has-winetricks   = scenario "wincompat-has-winetricks"   (builtins.elem pkgs.winetricks               cfg.environment.systemPackages);
  ...
  // {
    wincompat-runtime = pkgs.testers.runNixOSTest {
      ...
      nodes.machine = { ... }: { imports = [ ./wincompat.nix vmBase ]; };
      testScript = ''
