...
in
{
  quarto-has-extension = scenario "quarto-has-extension" (
    builtins.elem homePkgs.vscode-marketplace.quarto.quarto exts
  );
  quarto-qmd-association = scenario "quarto-qmd-association" (
    settings."files.associations"."*.qmd" == "quarto"
  );
  quarto-python-env-set = scenario "quarto-python-env-set" (
    cfg.home.sessionVariables.QUARTO_PYTHON != ""
  );
  quarto-preview-binding = scenario "quarto-preview-binding" (
    builtins.any (b: b.command == "quarto.preview") keybindings
  );
}
